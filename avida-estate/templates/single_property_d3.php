    ...
    <div class="pxp-content">
        ...
        <?php if ($photos[0] != '') { ?>
            <div class="<?php echo esc_attr($gallery_class); ?>">
                <div class="owl-carousel pxp-single-property-gallery-d3-stage">
                    <?php for ($i = 0; $i < count($photos); $i++) {
                        $p_photo_full = wp_get_attachment_image_src($photos[$i], 'pxp-full');
                        $p_photo_info = resideo_get_attachment($photos[$i]); ?>

                        <figure itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject" class="pxp-single-property-gallery-d3-item rounded-lg">
                            <a href="<?php echo esc_url($p_photo_full[0]); ?>" itemprop="contentUrl" data-size="<?php echo esc_attr($p_photo_full[1]); ?>x<?php echo esc_attr($p_photo_full[2]); ?>" class="pxp-cover rounded-lg" style="background-image: url(<?php echo esc_url($p_photo_full[0]); ?>);"></a>
